...
with Guass-Markov Assumption:\index{Guass-Markov Assumption}
...
\textbf{Degree of Freedom}\index{$ dof $/$ df $ (Degree of Freedom)} of a Quadric Form:\index{Degree of Freedom}
\begin{itemize}[topsep=2pt,itemsep=2pt]
...
\begin{point}
    ...
    \index{Gauss-Markov Thm.}Gauss–Markov Thm.: the OLS estimator has the lowest sampling variance within the class of linear unbiased estimators, i.e. OLS is the Best Linear Unbiased Estimator(BLUE).\footnote{This Thm. does \textbf{not }require normal error assumption.}\index{BLUE (Best Linear Unbiased Estimator)}
\end{point}
...
\subsubsection{Analysis of Variance: Multivariate}
    Sampling Notation see eqa(\ref{EqaSampleNotationOfMultiLinear}), still consider $ (p+1) $ -dim $ (\mathbf{1}_n,X_i) $ v.s. $ 1 $-dim $ Y $, and $ \beta=(\beta _0,\beta _1,\beta _2,\ldots,\beta _p) $\index{ANOVA (Analysis Of Variance)}
...
\end{rcode}    
    ...
    % Basic Diagnostics including to $ X $, to leverage, etc. see section \ref{SubSectionDiagnosticsForMonoModel}.
...
\subsubsection{Useful Diagnostics Plots}
        ...
        \item Quantile-Quantile Plots\index{QQ-Plot (Quantile-Quantile Plots)}\hypertarget{QQplot}{}: Examine the similarity  between distribution.
...
\end{rcode}
    ...
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        \item[$ {\color{red}\star}  $] Shapiro-Wilk Test\index{S­-W Test (Shapiro-­Wilk Test)} (Most Powerful)\footnote{Detail of S-W Test and K-S Test see \hyperlink{testofnormality}{Test of Normality} in sec.\ref{SubSectionIntroToNonParametricHypothesisTesting}}: To test $ H_0: \,\exists \sigma ^2, \,s.t. \,\varepsilon \sim N_n(0,\sigma ^2I_n)  $, denote 
        \[
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
   ...
   \item Diagnostics to \textbf{Influential}: Studentized DiFference in BETA estimates (DFBETAS)\index{DFBETAS (Studentized Difference in Beta Estimates)}. Use eqa(\ref{EqaDistributionOfMultiVariateBeta}), define
   \[
...
\item Pearson Correlation Coefficient $ r $ and Coefficient of Multiple Determination $ R^2 $\index{CMD (Coefficient of Multiple Determination)}:
    ...
    Pearson's $ r $:\index{Pearson's Correlation Coefficient}\index{Correlation Coefficient!Pearson's Correlation Coefficient}
    \begin{equation}
    ...
    CMD $ R^2 $:\index{Correlation Coefficient!CMD $ R^2 $}

    \begin{equation}
        R^2=\dfrac{\mathrm{SSR}}{\mathrm{SST}}=1-\dfrac{\mathrm{SSE}}{\mathrm{SST}}
    \end{equation}

    Adjusted $ R^2 $:\index{Correlation Coefficient!Adjusted $ R^2 $}
    \begin{equation}
...
\end{equation}
    ...
    \item Coefficient of Partial Correlation: Measures the strength of linear relation, $ \pm $ sign depend on posi./nega. correction.\index{Correlation Coefficient!Coefficient of Partial Correlation}
    \begin{equation}
...
\subsubsection{Remedies for Model Variable Selection}
...
\begin{equation}
    \hat{\beta }=\arg\min ||Y-X\beta ||_2 ^2, \text{ with }||\beta ||_1\leq s
\end{equation}

where $ s $ is a parameter correponding to $ \lambda  $.

\item Ridge Regression/Tikhonov Regularization:\index{Ridge Regression}\index{Tikhonov Regularization}

Penalty term: $ \lambda||\beta ||_2) $, where $ \lambda  $ is penalty parameter.
\begin{equation}
    \hat{\beta }=\arg\min (||Y-X\beta ||_2 ^2+\lambda||\beta ||_2)
\end{equation}

    or equivlantly expressed as 

\begin{equation}
    \hat{\beta }=\arg\min ||Y-X\beta ||_2 ^2, \text{ with }||\beta ||^2_2\leq s
\end{equation}
\end{itemize}
