\chapter{线性回归分析部分}\label{SecLinearRegressionAnalysis}
\begin{center}
...
\end{equation}
    ...
    Def. \index{Residuals}\textbf{Residuals}: distance from sample point to estimate point, to reflect how the sample points fit the model.
    \begin{equation}
...
\subsection{Statistical Inference to $ \beta _0 $, $ \beta _1 $, $ \sigma ^2 $}
...
\end{point}
    ...
    Further we can get $ var(\hat{\sigma }^2)=\mathbb{E}(\dfrac{1}{n-2}\sum_{i=1}^ne_i^2) $, where $ e_i^2\sim \sigma ^2\left( 1-\dfrac{1}{n}-\dfrac{(X_i-\bar{X})^2}{S_{XX}} \right)\chi^2 $
    \begin{equation}
...
\begin{point}
    Why we choose OLS to get regression coefficients?\index{Gauss-Markov Theorem}
\end{point}

    Gauss-Markov Theorem: the OLS estimator has the lowest sampling variance within the class of linear unbiased estimators, i.e. OLS is the \textbf{Best Linear Unbiased Estimator(BLUE)}.\footnote{This Theorem does \textbf{not }require normal error assumption.}\index{BLUE (Best Linear Unbiased Estimator)}
...
\end{rcode}
    ...
    \begin{point}
       Simultaneous Confidence Band (SCB)\index{SCB (Simultaneous Confidence Band)}\index{Confidence Region!Confidence Band}
    \end{point}
...
\end{align}
    ...
    \item Geometric/Algebraical: Use hyper-projection.\index{Projection Operator}
    \begin{equation}
...
\subsection{Statistical Inference to $ \beta  $, $ \sigma ^2 $}\label{SubSubSectionStatisticalInferenceInMultiLRA}
...
\begin{itemize}[topsep=2pt,itemsep=2pt]
    ...
    where \textbf{Hat Matrix}/Influence matrix/Projection matrix $ H=P_X=X(X'X)^{-1}X' $, with properties\index{Hat Matrix}
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        \item Symmertric: $ H^T=H $;
        \item Idempotence: $ H^2=H $\index{Idempotence}
...
\end{figure}
    ...
    Assumptions:\index{Gauss-Markov Assumption}
    \begin{equation}
        \begin{aligned}
            \text{Zero-Mean: }&\mathbb{E}(\epsilon_i|X_i)=0 \\
            \text{Homogeneity of Variance: }&var(\epsilon_i)=\sigma^2\\
...
\subsection{Useful Diagnostics Plots}
       ...
       \begin{itemize}[topsep=2pt,itemsep=0pt]
            \item BoxPlot: to examine the similarity of shape of distribution.\index{Box Plot}
...
\subsection{Diagnostics to $ X $ Distribution}\label{SubSecXDiagnostics}
    ...
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        \begin{itemize}[topsep=2pt,itemsep=2pt]
            ...
            Adjusted Skewness (MMSE):\index{Adjusted Skewness}
            \begin{equation}
            ...
            Fisher-Pearson coefficient of skewness: $ \dfrac{3(\mathrm{mean}-\mathrm{median})}{\sigma } $.\index{Fisher-Pearson Coefficient of Skewness}
            ...
            \begin{itemize}[topsep=2pt,itemsep=2pt]
                \item $ \hat{g}_2>0 $: Leptokurtic, heavy tail, slender;\index{Leptokurtic}
                \item $ \hat{g}_2<0 $: Platykurtic, light tail, broad.\index{Platykurtic}
            \end{itemize}
...
\end{rcode}
        ...
        \begin{itemize}[topsep=2pt,itemsep=2pt]
            \item Selection Bias: Not completely random sampling;\index{Selection Bias}
...
\end{rcode}
    \item The Assumption of \textbf{Equal Variances} / Homoscedasticity (齐方差性)\index{Homoscedasticity}\index{Heteroscedasticity}: 
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        \item Brown-Forsythe's Test\index{BF Test (Brown-Forsythe's Test)} (Modified Levene's test): For skewed sample, take the \textbf{mean} as \textbf{median}, more robust. 


        \item[$ {\color{red}\star} $] Breusch-Pagan Test:\index{BP Test (Breusch-Pagan Test)}
...
\end{rcode}
    ...
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        \item[$ {\color{red}\star}  $] Shapiro-Wilk Test\index{SW Test (Shapiro-Wilk Test)} (Most Powerful)\footnote{Detail of S-W Test and K-S Test see \hyperlink{testofnormality}{Test of Normality} in \autoref{SubSectionIntroToNonParametricHypothesisTesting}}: To test $ H_0: \,\exists \sigma ^2, \,s.t. \,\varepsilon \sim N_n(0,\sigma ^2I_n)  $, denote 
        \begin{equation}
        ...
        \item Kolmogorov-Smirnov Test\index{KS Test (Kolmogorov-Smirnov Test)}: 
        \begin{equation}
            D_n=\sum_{e}|F_n(e)-\Phi(e)|
        \end{equation}
        
        
        \item Cramér-von Mises Test\index{Cramér-von Mises Test}:
        \begin{equation}
            T=n\int_{-\infty}^\infty (F_n(e)-\Phi (e)) ^2\,\mathrm{d}\Phi(e)
        \end{equation}
        \item Anderson-Darling Test:\index{Anderson-Darling Test}
        \begin{equation}
            A^2-n\int _{-\infty}^\infty (F_n(e)-\Phi(e))^2\dfrac{1}{\Phi(e)(1-\Phi(e))} \,\mathrm{d}\Phi(e)
        \end{equation}
        \item Jarque-Bera Test \index{Jarque-Bera Test}, using skewness $ \hat{g}_1 $ and kurtosis $ \hat{g}_2 $ of $ \vec{e} $
        \begin{equation}
...
\subsection{Diagnostics to Influentials}\label{SubSubSectionDiagnosticsToInfluentials}
    ...
    The diagonal elements  of $ \hat{H} $ are \textbf{self-sensitivity} $ h_{ii} $\index{Self-sensitivity}
    \begin{equation}
        h_{ii}=X_i'(X'X)^{-1}X_i
    \end{equation}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    ...
    \end{equation}
    \item \index{Leverage}Diagnostics to \textbf{Leverage}: use hat matrix $ H $/self-sensitivity $ h_{ii} $.
    \begin{equation}
...
Mahalanobis Distance\index{Mahalanobis Distance} between $ X $ and $ Y $ as defined in \autoref{MahalanobisDistance}
\begin{equation}
...
\end{rcode}
    ...
    \textbf{Note}: Three types of SS \index{Type I \& II \& III SS}
\begin{table}[H]
    \centering
...
\subsection{Hypotheses Testing to Slope}\label{SubSubSectionGeneralLinearTest}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    ...
    \end{equation}
    \item Adjusted $ R^2 $:\index{Correlation Coefficient!Adjusted $ R^2 $}\index{Adjusted $ R^2 $}
    \begin{equation}
...
\end{equation}
    ...
    \item Coefficient of Partial Correlation $ \eta_k $: Measures the strength of linear relation, $ \pm $ sign depend on posi./nega. correction.\index{Correlation Coefficient!Coefficient of Partial Correlation $ \eta_k $}\index{Coefficient of Partial Correlation $ \eta_k $}
    \begin{equation}
...
\subsection{Diagnostics to Multi-colinearity}\label{SubSubSectionDiagnosticsToMultiCollinearity}
\index{Multi-colinearity}
\begin{itemize}[topsep=2pt,itemsep=2pt]
...
% Mallow's Cp, AIC,SBC/BIC,PRESS,
        ...
        And construct Mallow's $ C_p $: Estimation of $ \Gamma _p $
        \begin{equation}\label{EqaMallowsCp}
            C_p=\hat{\Gamma }_p=\dfrac{\mathbb{E}(\mathrm{SSE}(p))}{\hat{\sigma }^2}-(n-2p)
        \end{equation}

        where $ \mathrm{SSE(p)}=Y'(I-H_p)Y  $. When the model is unbiased, then we should have $ \mathbb{E}(\mathrm{SSE} (p))/\hat{\sigma }^2 \to n-p $. $ C_p$-$p $ plot could be used to pick a proper $ p $: 
        \begin{itemize}[topsep=2pt,itemsep=0pt]
...
\subsection{Variable Transformation}\label{SubSubSectionVarianceStablizeTransformation}
    ...
    \begin{point}
        Variance Stabilizing Transformations:\index{Variance Stabilizing Transformation}
    \end{point}
    ...
    \begin{point}
        Box-Cox Transformation:\index{Box-Cox Transformation}
    \end{point}
        ...
        \end{equation}
            ...
            where the Jacobi Matrix denoted in Geometric Mean \index{Geometric Mean}$ \mathrm{GM}(Y)=\prod_{i=1}^n Y_i^{1/n}$
            \begin{equation}
...
\begin{point}
    Variable Selection Methods\index{Variable Selection}
\end{point}

    Several Algorithm to search for best variable set:
\begin{itemize}[topsep=2pt,itemsep=0pt]
\item Exhaustive Search\index{Exhaustive Search} and \hyperlink{ModelValidationCriteria}{Test} (usually through Mallow's $ C_p $, see \autoref{EqaMallowsCp}): Used for $ p\leq \sim 30 $
\item Greedy Search: Get a locally optimal solution.\index{Greedy Search Algorithm}
\begin{itemize}[topsep=2pt,itemsep=2pt]
...
\begin{point}
    Regression with Penalty Term / Regularization\index{Regularization}
\end{point}
...
\end{point}
    ...
    Add smooth/penalty function. e.g. \lstinline|loess| (Locally Regression), \lstinline|lowess| (Locally Weighted ScatterPlot Smoother), Regression Tree.\index{LOWESS (Locally Weighted ScatterPlot Smoother)}\index{LOESS (Locally Regression)}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
       \item Standardized Regression Model\index{Standardized Regression Model}
...
\end{lstlisting}
\end{rcode}
    \item Polynomial Regression Model\index{Polynomial Regression Model}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    \item Cell means model\index{Cell Means Model} solution for $ c_i=\delta _{i1} $, i.e. $ \tau_1=0 $. In this case we directly express $ \mu _i=\mu +\tau_i $
    \begin{align*}
        \hat{\mu }_i=&\bar{Y}_i=\dfrac{1}{n_i}\sum_{j=1}^{n_i}Y_{ij}\\ 
    \end{align*}
    \item Factor effect solution\index{Factor Effect Model} for $ c_i=1 $, i.e. $ \sum_{i=1}^r \tau _i=0 $ (used most often, and is \textbf{used in the following parts in this chapter}).
\begin{align}
...
\begin{point}
    One-Way ANOVA\index{ANOVA (Analysis of Variance)}
\end{point}
...
\end{point}
    ...
    where $ \phi $ with $ \sum_{i=1}^r\xi _i=0 $ is called a \textbf{contrast}\index{Contrast}. Assume there are $ m $ estimator $ \phi _k $, $ k=1,2,\ldots,m $% , $ m\leq \dfrac{r(r-1)}{2} $
...
\begin{itemize}[topsep=2pt,itemsep=0pt]    
    ...
    \item Bonferroni's Confidence Region\index{Fisher's LSD (Fisher's Least Significant Difference)} for $ \mathop{\phi}\limits_{m\times 1}  $, using result in \autoref{EqaConfidenceRegionUsingBonferroni}
    \begin{equation}
    ...
    \item Scheff\`{e}'s Confidence Region for $ \mathop{\phi }\limits_{1\times 1} $:\index{Scheff\`{e}'s Method}
    \begin{equation}
         R(\phi)=\sum_{i=1}^r\xi_i\bar{Y}_i\pm \hat{\sigma }\sqrt{(r-1)F_{r-1,n_T-r,\alpha }}\sqrt{\sum_{i=1}^r\dfrac{\xi _{ki}^2}{n_i}}
    \end{equation}
    
    \item \index{Tukey's HSD (Tukey's Honestly Significant Difference)}Tukey's HSD Confidence Region for $ \mathop{\phi }\limits_{1\times 1} $, under condition $ n_1=\ldots=n_r=n $: focus on estimating $ \tau_i-\tau_j $
    
    \begin{itemize}[topsep=2pt,itemsep=0pt]
        \item Def.: studentized range \hypertarget{TukeyStudentizedRangeDistribution}{}distribution\index{Studentized Range Distribution}: for $ Z_1,\ldots,Z_n $ i.i.d. $ \sim N(0 ,1) $, $ mW^2\sim \chi _m^2 $, then 
...
\end{itemize}
    ...
    \textbf{Comment}: Scheff\`{e} is more conservative, i.e. shorter. If confidence interval does not include $ 0 $, we can say they are significantly different. More about theory behind these confidence region see \autoref{SubSubSectionDoEOneWayANOVA}-\hyperlink{DoEMultipleComparison}{Multiple Comparison}
...
\end{point}
\index{Scaled Exponential Family}
...
\begin{point}
    Dependent Mode Generalize: Link Function\index{Link Function}
\end{point}
...
\end{equation}
    ...
    such condition is called \textbf{Canonical Link}\index{Canonical Link} of generalized linear model, such choise of link function makes $ x'\beta  $ the canonical parameter in model.
    \begin{equation}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    ...
    \begin{equation}
        \mu _i=\eta_i
    \end{equation}
    \item Logistic Model:\index{Logistic Regression} $ B(n,\pi) $, $ g(x)=\mathrm{logit}(x)=\ln\dfrac{x}{1-x}\Leftarrow   $$ g^{-1}(y)=\mathrm{logistic}(y)=\dfrac{1}{1+e^{-y}}  $
