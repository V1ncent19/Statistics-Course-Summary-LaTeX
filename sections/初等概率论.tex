...
\subsubsection{Covariance and Correlation}\label{SubSubSectionCovarianceAndCorrelation}
    ...
    And (Pearson's) Correlation Coefficient\index{Pearson's Correlation Coefficient}\index{Correlation Coefficient!Pearson's Correlation Coefficient}
    \begin{equation}
        r=\rho_{X,Y}=corr(X,Y)=\frac{cov(X,Y)}{\sqrt{var(X)var(Y)}}
    \end{equation}

    Remark: correlation $\nRightarrow$ cause and effect. Detail on causal effect topic see section \ref{SecCausalInference}.
...
\begin{itemize}
    \item WLLN\index{LLN (Law of Large Number)}\index{WLLN (Weak Law of Large Number)}
\begin{equation}    \frac{1}{n}\sum X_i\xrightarrow[]{p} E(X_1)
\end{equation}
\item SLLN\index{SLLN (Strong Law of Large Number)}
\begin{equation}    \frac{1}{n}\sum X_i\xrightarrow[]{\text{a.s.}}  C
\end{equation}
\item CLT\index{CLT (Central Limit Thm.)}
...
\subsection{Multivariate Normal Distribution}\label{SubsectionDerivationMultivariateNormal}
    General Case and more discussion see section\ref{SubSubSectionMultivariateNormalDistribution}.

    Distribution of Normal $ X(\mu ,\sigma ^2) $:\index{Distribution!Normal Distribution}
    \[
