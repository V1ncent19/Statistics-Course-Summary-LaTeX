...
\subsection{Axioms of Probability}
    ...
    \begin{align}
       \mathbb{P}\left( A \right) :=\begin{cases}
        ...
        \end{cases}   
    \end{align}
    \index{Classic Model}\index{Geometric Model}
...
\end{equation}
\item Normalization\index{Normalization}\footnote{Note: In other sections when dealing with not-yet-normalized distribution (say in Bayesian statistics), I usually use $ Z $ as the normalize constant, following the tradition in statistical physics where $ Z $ is the partition function.
\begin{align}
...
\end{point}
    ...
    \begin{itemize}
        ...
        \end{align}
        \item Monotonicity\index{Monotonicity}
        \begin{equation}    
            \mathbb{P}(A)\leq \mathbb{P}(B)\quad \text{for}\, A\subset B
        \end{equation}
        \item Finite Subadditivity (Boole Inequality)\index{Inequality!Boole Inequality}\index{Boole Inequality}\index{Finite Subadditivity}
        \begin{equation}    
...
\subsection{Conditional Probability}
    ...
    Def. \textbf{Conditional Probability} of $B$ given $A$:\index{Conditional Probability}
    \begin{equation}    
...
\end{point}
        ...
        \begin{itemize}
        \item Multiplication Formula\index{Multiplication Formula}
        \begin{equation}    
            \mathbb{P}(\bigcap_{i=1}^n A_i)=\mathbb{P}(A_1)\prod_{i=2}^n \mathbb{P}(A_i|A_1\cap A_2\cap \cdots\cap A_{i-1})    
        \end{equation}
        \item Total Probability Theorem\index{Total Probability Theorem}
        \begin{equation}    
...
\subsection{Independency}
    Statistical Independency is defined as:\index{Independence}
    \begin{equation}    
...
\subsection{Random Variable}
    ...
    \begin{itemize}
        ...
        \end{equation}
        \item $p$-fractile\index{Fractile!$ p $-fractile}\index{p-fractile@$ p $-fractile}
...
\subsection{Random Vector}
    ...
    $k$-dimensional Marginal Distribution\index{Marginal Distribution}: For $1\leq k<n$ and index set $S_k=\{i_1.\ldots,i_k\}$, distribution of $\vec{X}=(X_{i_1},X_{i_2},\ldots,X_{i_k})$
...
\begin{itemize}
    ...
    \item Law of Total Expectation\index{Law of Total Expectation}\begin{equation}
    ...
    \end{equation}
    \item Conditional Expectation\index{Conditional Expectation}
    \begin{equation}
...
\subsection{Variance $ var(\, \cdot \, ) $}
    Variance of r.v. $X$: \index{Variance}
    \begin{equation}
...
\begin{itemize} 
    ...
    \end{equation}
    \item Law of Total Variance\index{Law of Total Variance}\begin{equation}
        var(X)=\mathbb{E}[var(X|Y)]+var[\mathbb{E}(X|Y)]
    \end{equation}
\end{itemize}

    Standard Deviation\index{Standard Deviation} def. as :
...
\subsection{Probability Generating Function}
    PGF\index{PGF (Probability Generating Function)}: used for non-negative, integer $X$, which is the $ z $-transformation of $ p_X $\index{z-transformation@$ z $-transformation}
    \begin{equation}
...
\subsection{Moment Generating Function}
    MGF\index{MGF (Moment Generating Function)}: used for non-negative $ X $, which is the Laplace transformation of $ f_X $.\index{Laplace Transformation}
    \begin{equation}
...
\subsection{Characteristic Function}
    C.F \index{Characteristic Function}is actually the Fourier Transform (FT) of $f_X$.\index{FT (Fourier Transform)}\footnote{More about FT see \autoref{SubSubSectionFourierAndConvolution}.}
    \begin{equation}
    ...
    \end{equation}
    \item Inverse (Fourier) Transform \index{Inverse Fourier Transform}
    \begin{equation}
...
\subsection{Convergence Mode}
    \index{Convergence}\index{Almost Sure Convegence}\index{Lp Convergence@$ L_p $ Convergence}
    \begin{equation}
        \begin{cases}
    ...
    \end{equation}
        ...
        Useful Theorem:
        \begin{itemize}[topsep=2pt,itemsep=0pt]
            \item Continuous Mapping Theorem: For continuous function $g(\cdot)$\index{Continuous Mapping Theorem}\hypertarget{ContinuousMapping}{}
            ...
            \end{enumerate}
            \item Slutsky's Theorem\index{Slutsky's Theorem}: For $X_n\xrightarrow[]{\mathrm{d}}X,Y_n\xrightarrow[]{\mathrm{p}}c$
                \begin{enumerate}[topsep=2pt,itemsep=2pt]   
...
\end{equation}
\item CLT\index{CLT (Central Limit Theorem)}: For $ X_i $ i.i.d. $ \sim f_X $, with $ \mathbb{E}\left[ X_i \right] =\mu <\infty $, $ var(X_i)=\sigma ^2<\infty $
\begin{align}
...
\item de Moivre-Laplace Theorem\index{de Moivre-Laplace Theorem} is a special case of CLT at $ S_n\sim B (n,p) $
\begin{equation}    \mathbb{P}(k\leq S_n\leq m)\approx \Phi(\frac{m+0.5-np}{\sqrt{npq}})-\Phi(\frac{k-0.5-np}{\sqrt{npq}})
\end{equation}
\item Stirling Eqa. derived from CLT\index{Stirling Equation}
...
\begin{itemize}
    \item Cauchy-Schwarz Inequality\index{Inequality!Cauchy-Schwarz Inequality}\index{Cauchy-Schwarz Inequality}
    \begin{equation}
        \left\vert \mathbb{E}(XY) \right\vert\leq \sqrt{\mathbb{E}(X^2)\mathbb{E}(Y^2)}
    \end{equation}

    \item Bonferroni Inequality\index{Inequality!Bonferroni Inequality}\index{Bonferroni Inequality}
\begin{equation}    \mathbb{P}(\bigcup_{i=1}^n A_i)\geq \sum_{1\leq i\leq n}  \mathbb{P}(A_i)+\sum_{1\leq i <j\leq n}  \mathbb{P}(A_i\cap A_j)
\end{equation}
    \item Markov Inequality\index{Inequality!Markov Inequality}\index{Markov Inequality}
...
\end{equation}
    ...
    \item Chebyshev Inequality\index{Chebyshev Inequality}
...
\end{equation}
    ...
    \item Jensen Inequality\index{Inequality!Jensen Inequality}\index{Jensen Inequality}: For convex function $h(x)$:\footnote{Or equivalently for concave function $ \tilde{h}(x) $:
    \begin{align}
...
\end{equation}
    ...
    \end{align}
    \item Hoeffding Inequality\index{Hoeffding Inequality}\index{Inequality!Hoeffding Inequality}: with independent r.v. sequence $ X_i\in [a_i,b_i] $, and $ S_n:=\sum_{i=1}^nX_i $
    \begin{align}
...
\section{Multivariate Normal Distribution}\label{SubsectionDerivationMultivariateNormal}
    ...
    Distribution of Normal r.v. $ X\sim N(\mu ,\sigma ^2) $:\index{Distribution!Normal Distribution}
    \begin{align}
...
%    Then $Y_1,\cdots,Y_n\sim N$ 
    ...
    \subsection{Distributions of Function of Normal Variable: $\chi^2,$ $t\,\& \,F$}\label{chi2_t_F_properties}
        ...
        \begin{itemize}
            \item $\chi^2$ Distribution\index{chi2 Distribution@$ \chi^2 $ Distribution}: Def. $\chi^2$ distribution with degree of freedom $n$:\index{Distribution!$ \chi^2 $ Distribution}
            \begin{equation}        
            ...
            \end{itemize}
            \item $t$ Distribution:\index{t Distribution@$ t $ Distribution} Def. $t$ distribution with degree of freedom $n$:\index{Distribution!$ t $ Distribution}
            \begin{equation}        
            ...
            \item $F$ Distribution\index{F Distribution@$ F $ Distribution}: Def. $F$ distribution with degree of freedom $m$ and $n$:\index{Distribution!$ F $ Distribution}
            \begin{equation}        
