...
\end{equation}
    ...
    \begin{minipage}{0.9\linewidth}
        $ \Delta $ \textbf{NOTE:} Here $ g(\theta ) $ is the parameter, which is fixed, while confidence interval, as a function of data, is random. So all the probabilities discussed below are \textbf{Probability that the interval covers the real value}, rather that the real value falls in the interval. There is a huge difference.
    \end{minipage}
    }
...
\end{point}
        ...
        \item[$ \bm{\Delta } $] $p$-value: probability to get larger bias (or simply `more extreme data') than observed $\vec{x}_0$ \uline{with $H_0$ as ground truth.}
