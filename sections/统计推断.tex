...
\end{center}
    ...
    \textbf{Statistical Inference}: Given sample $ X=(x_{1},x_{2},\ldots,x_{n})  $, we want to estimate some features of the population. This part focus on parametric statistical inference, thus our task is to estimate/testing parameters.\index{Statistical Inference}
...
\section{Statistical Model and Statistics}\label{SectionStatisticalModelandStatistics}
    Random sample comes from population $X$. In parametric model case, we have population distribution family:\index{Distribution Family}
...
\end{point}
    ...
    \begin{itemize}
        \item Sample mean (Consider $X_i$ i.i.d.)\index{Mean}
        \begin{equation}
        ...
        \end{equation}
        \item Sample moments\index{Moment}
        \begin{itemize}
            \item Origin moment
            \begin{equation}
            ...
            \end{equation}
        \end{itemize}
        \item Pearson's Correlation Coefficient $ r $\index{Correlation Coefficient!Pearson's Correlation Coefficient $ r $}\index{Pearson's Correlation Coefficient $ r $}
        \begin{align}
        ...
        \end{equation}
        \item Sample coefficient of variation\index{Coefficient of Variation}
        \begin{equation}
            \hat{\nu}=\frac{S}{\bar{X}}    
        \end{equation}
        \item Skewness and Kurtosis\index{Skewness}\index{Kurtosis}
        \begin{equation}
...
\subsection{Sufficient and Complete Statistics}\label{SubSectionSufficient_CompleteStatistics}
    ...
    \begin{itemize}
        ...
        Properties
        \begin{itemize}
            \item \textbf{Factorization Theorem}\index{Factorization Theorem} $T(\vec{X})$ is sufficient \textbf{if and only if} $f_{\vec{X}}(\vec{x};\theta)=f(\vec{x};\theta)$ can be written as 
            \begin{equation}
        ...
        \item[$\blacktriangleright$] A \textbf{Complete Sufficient Statistics}: with both sufficient and complete properties satisfied.
        ...
        \begin{itemize}[topsep=2pt,itemsep=0pt]
            ...
            \item If minimal sufficient statistic exists, then any complete statistic is also minimal sufficient $  \Rightarrow  $ complete \& sufficient.
        \end{itemize}
...
\subsection{Optimal Criterion}\label{SubSectionOptimalCriterion}
    ...
    \begin{itemize}
        \item Unbiasedness\index{Unbiasedness}: say $\hat{\theta }(\vec{X})$ or $\hat{g}(\vec{X})$ is unbiased estimator of $\theta$ or $g(\theta)$, if
        \begin{equation}
            \mathbb{E}(\hat{\theta})=\theta,\quad \mathbb{E}(\hat{g})=g(\theta)
        \end{equation}

        Otherwise, say $\hat{\theta}$ or $\hat{g}$ is biased. Define \textbf{Bias}:\index{Bias}
        \begin{align}
        ...
        Asymptotic unbiasedness with $ n $ as sample size\index{Asymptotic Unbiasedness}:
        \begin{equation}
        ...
        \item Minimum Mean Squared Error (MMSE)\index{MMSE (Minimum Mean Squared Error Estimator)}: Most efficient in the sense with bias-variance trade-off\index{Bias-Variance Trade-Off}. More about Minimum MSE estimation see \autoref{SubSecMMSE}.
        ...
        \item (Weak) Consistency\index{Consistency}
        \begin{equation}
...
\subsection{Method of Moments}\label{SubSectionMoM}
    ...
    Then its \textbf{Moment Estimate} $\hat{g}(\vec{X})$ is\index{Moment Estimate}
\begin{equation}
...
\end{point}
    ...
    \item \textbf{Sufficient and Complete Statistic Method}. This method relies on two theorems:
    \begin{itemize}[topsep=2pt,itemsep=0pt]
        \item \textbf{Rao-Blackwell Theorem}\index{Rao-Blackwell Theorem}: For $T(\vec{X})$ sufficient statistic, $\hat{g}(\vec{X})$ unbiased estimate of $g(\theta)$, then 
        \begin{equation}
        ...
        \item \textbf{Lehmann-Scheffé Theorem}\index{Lehmann-Scheffé Theorem}: For $ T(\vec{X}) $ sufficient \& complete, $ \hat{g}(T()\vec{X}) $ an unbiased estimate of $ g(T(\theta )) $, then $ \hat{g}(T(\vec{X})) $ is the unique UMVUE.
        ...
        \begin{align*}
             \mathbb{E}\left[ \tilde{g}(\vec{x})|T(\vec{x}) \right] \text{ the unique UMVUE for } g(\theta ) 
        \end{align*}
    ...
    \item \textbf{Cramer-Rao Inequality}\index{CR Inequality (Cramer-Rao Inequality)}\index{Inequality!CR Inequality}
...
\subsection{OLS, MoM, and MLE in Linear Regression}\label{SubSectionMoM_MLE_LinearRegression}
    \textbf{Note:} More detailed knowledge see \autoref{SecLinearRegressionAnalysis} Linear Regression Analysis.\index{Linear Regression}
...
\end{point}
    ...
    Basic Assumptions (Guass-Markov Assumption):\index{Gauss-Markov Assumption}
    \begin{align}
    ...
    \begin{enumerate}
        ...
        (Express in Matrix Notation \autoref{EqaMatrixRepreOfSSE}, so that it can be generalized to multidimensional case)\index{SSE (Error Sum of Squares)} SSE can be expressed as the \textbf{Eucliean Distance }between $ \{y_i\} $ and $ \{\hat{\beta}_0+\hat{\beta}_1x_i\}  $, i.e.
        \begin{equation}
...
\subsection{Kernel Density Estimation}\label{SubSectionKernelDensityEstimation}
    \index{Kernel Density Estimation}Given random sample $\{X_i\}$. Def. Empirical CDF.\index{ECDF (Empirical CDF)}
...
\begin{point}
        Useful Kernel Functions\index{Gaussian Kernel}
\end{point}
...
\subsection{Pivot Variable Method}\label{SubSectionPivotVariableMethod}
    ...
    Def. \textbf{Pivot Variable}\index{Pivot Variable Method} $T$, satisfies: 
    \begin{itemize}[topsep=2pt,itemsep=0pt]
...
\end{align}
    ...
    \begin{enumerate}[topsep=2pt,itemsep=2pt]
        ...
        When $\sigma_1^2\neq\sigma^2_2$ and unknown, estimate $\mu_1-\mu_2$: Behrens-Fisher Problem\index{Behrens-Fisher Problem}, remains unsolved\footnote{An approximation is Welch-Satterthwaite equation. Detail see \autoref{SubSubSectionDoETwoSampleComparison}.}, but we can deal with simplified cases. 
        ...
        \begin{table}[H]
            ...
            \begin{tabularx}{\linewidth}{|c|c|Y|}
                ...
                Estimation& Pivot Variable & Confidence Interval\\
                \hline
                $\mu_1,\mu_2$ known, estimate $\dfrac{\sigma^2_1}{\sigma_2^2}$&$F=\dfrac{S_{\mu_2}^2}{S_{\mu_1}^2}\dfrac{\sigma_1^2}{\sigma^2_2}\sim F_{n,m}$&\makecell{$\left[\dfrac{S_{\mu_1}^2}{S_{\mu_2}^2}\dfrac{1}{F_{m,n,\frac{\alpha}{2}}},\dfrac{S_{\mu_1}^2}{S_{\mu_2}^2}\dfrac{1}{F_{m,n,1-\frac{\alpha}{2}}}\right]$\\or $\left[\dfrac{S_{\mu_1}^2}{S_{\mu_2}^2}{F_{m,n,\frac{\alpha}{2}}},\dfrac{S_{\mu_1}^2}{S_{\mu_2}^2}F_{n,m,\frac{\alpha}{2}}\right]$}\\
                \hline
                $\mu_1,\mu_2$ unknown, estimate $\dfrac{\sigma^2_1}{\sigma_2^2}$&$F=\dfrac{S_Y^2}{S_X^2}\dfrac{\sigma_1^2}{\sigma^2_2}\sim F_{n-1,m-1}$&\makecell{$\left[\dfrac{S_X^2}{S_Y^2}\dfrac{1}{F_{m-1,n-1,\frac{\alpha}{2}}},\dfrac{S_X^2}{S_Y^2}\dfrac{1}{F_{m-1,n-1,1-\frac{\alpha}{2}}}\right]$\\or $\left[\dfrac{S_X^2}{S_Y^2}{F_{m-1,n-1,\frac{\alpha}{2}}},\dfrac{S_X^2}{S_Y^2}F_{n-1,m-1,\frac{\alpha}{2}}\right]$}\\
                \hline
            \end{tabularx}
        \end{table}
        \end{enumerate}
...
\subsection{Fisher Fiducial Argument*}\label{SubSectionFisherFiducialArgument}
    (Not complete yet)
    \index{Fisher Fiducial Argument}
...
\end{point}
        ...
        \item[$ \bm{\Delta } $] \index{p-value@$ p $-value}$p$-value: probability to get larger bias (or simply `more extreme data') than observed $\vec{x}_0$ \uline{if $H_0$ as ground truth, and $ H_1 $ as alternative.}
...
\begin{point}
    Limiting Distribution of LRT: Wilks' Theorem\index{Wilk's Theorem}
\end{point}
...
\subsection{Uniformly Most Powerful Test}\label{SUbSectionUMP}
    \index{UMP Test (Uniformly Most Powerful Test)}Idea: Neyman-Pearson Principle: control $\alpha$, find $\min\beta$. i.e. control $\alpha$, find $\max\pi(\theta)$
    ...
    \textbf{Neyman-Pearson Lemma}\index{Neyman-Pearson Lemma}: For $\vec{X}=(X_1,X_2,\ldots,X_n)$ i.i.d. from $f(\vec{x};\theta)$. 
...
\end{point}
    ...
    Using Neyman-Pearson we have only two-points test $ H_0:\theta =\theta _0 \longleftrightarrow H_1:\theta =\theta _1 $. With certain condition we could upgrade two-points two one-sided test $ H_0:\theta \leq \theta _0 \longleftrightarrow H_1:\theta >\theta _0 $ by \textbf{Karlin-Rubin Theorem}\index{Karlin-Rubin Theorem}. 

    \begin{enumerate}[topsep=2pt,itemsep=0pt]
        \item Monotone Likelihood Ratio Condition \index{MLR Condition (Monotone Likelihood Ratio Condition)}: If $ \forall \theta > \tilde{\theta } $ we have
        \begin{align*}
            \dfrac{ L(\theta ) }{ L(\tilde{\theta }) } \text{ is monotone in sufficient stat} T(\vec{X})  
        \end{align*}
        then we say that the MLR condition is satisfied.
        \item With MLR condition, we could upgrade two-points test to one-sided test $ H_0: \theta <\theta _0 \longleftrightarrow H_1:\theta >\theta _0 $. We would have a UMP test of form 
        \begin{align*}
    ...
    \textbf{Note:} Upgrade to two-sided test $ H_0:\theta =\theta _0 \longleftrightarrow H_1:\theta \neq \theta _0 $ is not always possible. e.g. $ H_0:\mu =\mu _0 \longleftrightarrow H_1:\mu \neq \mu _0 $ for $ X\mathop{ \sim  }\limits^{i.i.d.} N(\mu ,\sigma_0^2 \text{ known})  $ does not have UMP test.
...
\index{Non-parametric Hypothesis Testing}
    Motivation: Usually distribution form unknown, cannot use parametric hypothesis testing.

    Useful Method:
    \begin{itemize}
        \item Sign Test\index{Sign Test}: Used for paired comparison $\vec{X}=(X_1,X_2,\ldots,X_n)$, $\vec{Y}=(Y_1,Y_2,\ldots,Y_n)$.
        ...
        Def. \textbf{Sum of Wilcoxon Signed Rank}\index{Sum of Wilcoxon Signed Rank}: 
        \begin{equation}
...
\end{rcode}
        ...
        \textbf{Goodness-of-Fit Test}: Reject $H_0$ if $p(d_0)<\alpha$.
            ...
            Pearson Theorem: For $K_n$ defined as \autoref{Pearson_chi_test_differenceKn}, then under $H_0$:\index{Pearson's Theorem}
            \begin{equation}
...
\end{rcode}
    ...
    \begin{itemize}
        \item Kolmogorov-Smirnov Test\index{KS Test (Kolmogorov-Smirnov Test)}: Assume $\vec{X}$ form population CDF $F(x)$, test $H_0:F(x)=F_0(x)$(where can take $F_0=\Phi$ or some other known CDF).
        ...
        \item Shapiro-Wilk Test:\index{SW Test (Shapiro-Wilk Test)}
