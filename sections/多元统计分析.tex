...
\subsubsection{Review: Some Matrix Notation \& Lemma}\label{SubSubSectionMatrixNotationAndLemma}
    ...
    \begin{itemize}[topsep=6pt,itemsep=4pt]
        ...
        \hypertarget{MatrixDifferenciation}{Properties (under denominator-layout):}\footnote{More matrix diffrenciation equation see book \cite{线性回归分析ref2} P49.}
        ...
        \item \hypertarget{NormDefinition}{Norm}: Denote $ x=[x1,x2,\ldots,x_n]^T\in\mathbb{R}^n $, then the $ \ell_p $-norm of $ x $ is\index{Norm}
        \[
        ...
        Useful norm:
        \begin{itemize}[topsep=2pt,itemsep=2pt]
            \item $ \ell_0$-norm: \# of none 0 elements in $ x $;
            \item $ \ell_1 $-norm: $ ||x||_1=\sum_{i=1}^n|x_i| $;
            \item $ \ell_2 $-norm/Euclidean norm: $ ||x||_2=\sqrt{\sum_{i=1}^n x_i^2}  $.
        \end{itemize}
...
\begin{itemize}[topsep=2pt,itemsep=2pt]
    ...
    Where $ W_p(n-1,\Sigma ) $ is Wishart Distribution,\index{Distribution!Wishart Distribution} details as follows:
...
\begin{itemize}[topsep=2pt,itemsep=2pt]
    ...
    \[
        \bar{X}_1-\bar{X}_2\xrightarrow[]{\mathscr{L}} N(\mu_1-\mu _2,\dfrac{1}{n_1}\Sigma _1+\dfrac{1}{n_2}\Sigma _2) \qquad\quad \dfrac{1}{n_1}S_1+\dfrac{1}{n_2}S_2\xrightarrow[]{\mathscr{L}} \dfrac{1}{n_1}\Sigma _1+\dfrac{1}{n_2}\Sigma _2
    \]
...
\end{point}
    ...
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        i.e. data will become `sparse' in high dimension data -> difficult to extract information.}
    \end{itemize}
...
\subsubsection{Population Principal Component}
    ...
    To cancel out the influence due to scale, we can also obtain standardized PC from $ Z=(V)^{-1/2}(X_\mu) $, where $ V $ is standard deviation matrix as def. in eqa(\ref{EqaStandardDeviationMatrix}).
...
\subsubsection{Sample Principal Component}
    For sample matrix $ X $ denoted in eqa(\ref{EqaNPSampleMatrixNotation}), with cov. matrix $ S $ in eqa(\ref{EqaSampleCovarianceMatrix}). Then conduct the above spectrum decomposition to $ S $ to get sample PCs.
    ...
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        \item Independence:
        \[
            \hat{\lambda }_i\parallel \hat{e}_i 
        \]
...
\subsubsection{Orthogonal Factor Model}
    ...
    \begin{equation}
        \begin{aligned}
            E(F)&=0&cov(F)&=I_n\\
            E(\varepsilon )&=0&cov(\varepsilon )&=\Psi =diag\{\psi _1,\psi _2,\ldots ,\psi _p\}\\
            \varepsilon \parallel& F\Leftrightarrow cov(F,\varepsilon )=0&&
        \end{aligned}
    \end{equation}
...
\subsubsection{Canonical Variate Pair}
    ...
    Similarly, the $ k^{\mathrm{th}} $ canonical pair $ (U_k,V_k) $ satisfy the same criterion as eqa(\ref{EqaFirstCanonicalVariatePair}) but with $ a_k\in \mathrm{span}\{a_1,\ldots,a_{k-1}\}^\perp,$ $b_k\in\mathrm{span}\{b_1,\ldots,b_{k-1}\}^\perp $, $ k\leq \min\{p,q\} $.
...
\begin{itemize}[topsep=2pt,itemsep=2pt]
    ...
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        \item Conditional Probability $ P(i|j) $, $ i,j=1,2 $: probability that a $ \pi_j $ object falls in region $ R_i $
    \end{itemize}
...
\begin{itemize}[topsep=2pt,itemsep=2pt]
    ...
    \[
        \mathrm{ECM}=c(2|1)P(2|1)p_1+c(1|2)P(1|2)p_2 
    \]
    
    For two-category problem, $ R_1 $, $ R_2 $ can be determined as
    \begin{align*}
        R_1=&\arg \dfrac{f_{\pi_1}(x)}{f_{\pi_2}(x)}\geq \dfrac{c(1|2)}{c(2|1)}\dfrac{p_2}{p_1}\\
        R_2=&\complement_{R_x}^{R_1}=\arg \dfrac{f_{\pi_1}(x)}{f_{\pi_2}(x)}<\dfrac{c(1|2)}{c(2|1)}\dfrac{p_2}{p_1}
    \end{align*}
    ...
    \[
        \mathrm{TPM}=P(\mathrm{misclass})=P(2|1)p_1+P(1|2)p_2 
    \]
    ...
    \[
        P(X\in\pi_i|X=x_0)=\dfrac{p_i f_{\pi_i(x_0)}}{p_1f_{\pi_1}(x_0)+p_2f_{\pi_2}(x_0)},\, i=1,2
    \]
...
\subsubsection{Linear \& Quadratic Discriminant Analysis}
    Now take two-category ECM criterion as example. An estimation to $ P(1|2),\,P(2|1) $, i.e. to $ f_{\pi_1},\,f_{\pi_2} $ is needed.
    ...
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        then 
        \begin{align*}
            R_1=&\arg(\mu_1-\mu _2)'\Sigma ^{-1}x-\dfrac{1}{2}(\mu _1-\mu _2)'\Sigma ^{-1}(\mu _1-\mu _2)\geq \ln\left(\dfrac{c(1|2)}{c(2|1)}\dfrac{p_2}{p_1}\right)\\
            R_2=&\arg(\mu_1-\mu _2)'\Sigma ^{-1}x-\dfrac{1}{2}(\mu _1-\mu _2)'\Sigma ^{-1}(\mu _1-\mu _2)< \ln\left(\dfrac{c(1|2)}{c(2|1)}\dfrac{p_2}{p_1}\right)
        \end{align*}

        Note that $  \mathrm{L.H.S.} $ is a linear combination of $ x $, thus called LinearDA.

        Sample estimation to $ \Sigma  $: use pooled variance in eqa(\ref{EqaPooledVarianc}).
...
\subsubsection{Fisher's Discriminant Analysis}
    ...
    \begin{align*}
        \psi =&\dfrac{\sum\limits_{i=1}^g(\mu_{iY}-\mu_Y)^2}{\sigma ^2_Y}=\dfrac{a'\left(\sum\limits_{i=1}^g(\mu_i-\mu)(\mu_i-\mu)'\right)a}{a'\Sigma a}\\
        =&\dfrac{a'B_\mu a}{a'\Sigma a}
    \end{align*}
...
\begin{itemize}[topsep=2pt,itemsep=2pt]
    ...
    \[
        \mathrm{TPM}=p_1P(2|1)+p_2P(1|2)=p_1\int _{R_2}f_{\pi_1}(x) \,\mathrm{d}x+p_2\int _{R_1}f_{\pi_2}(x) \,\mathrm{d}x
    \]
...
\subsection{Clustering Analysis}
    \index{CA (Clustering Analysis)}\index{Clustering Analysis}
...
\subsubsection{Agglomerative Clustering Algorithm}\
    \index{Clustering Analysis!Agglomerative Clustering Algorithm}\index{Clustering Analysis!Hierarchical Clustering}
    Hierarchical clustering: start with individual points and combine them to form groups. Agglomerative clustering algorithm is conducted by repeating following steps:
    \begin{enumerate}[topsep=2pt,itemsep=2pt]
        \item Start: All $ k=n $ points are individual clusters;
        ...
        \item merge closest pairs of clusters(or points) to form larger clusters, and now number of clusters $ k <- k-1 $;
        \item Go back to step.2 and repeat, until there is only one cluster left.
    \end{enumerate}
...
\end{point}
    ...
    \noindent Choice of distance functional $ D(\cdot,\cdot) $:
\begin{itemize}[topsep=2pt,itemsep=2pt]
...
\end{itemize}
    ...
    \noindent Location choice of cluster:    
    ...
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        ...
        \item Group average: $ <D(a\in A,b\in B)> $
    \end{itemize}
    
    \noindent Note: pros-and-cons of agglomerative clustering algorithm
    \begin{itemize}[topsep=2pt,itemsep=2pt]
...
\subsubsection{$ k $-Means Clustering Algorithm}
    \index{Clustering Analysis!$ k $-Means Clustering Algorithm}
    Assume we have a preset number of clusters $ k $, we can use $ k $-means clustering.
...
\begin{enumerate}[topsep=2pt,itemsep=2pt]
    ...
    \end{itemize}
    \item For all points $ i $, calculate its distance from the $ l^\mathrm{th} $ centroid $ D(i,l) $, and classify each $ i $ point to the nearest centroid cluster;
    \item Re-calculate the centroid of new $ k $ clusters;
    \item Repeat step 4,5 until convergence.(Convergence criterion can be e.g. $ <\sum_i D(i\in g_l,l)>\to \mathrm{const} $)
...
\subsubsection{}
\index{Clustering Analysis!Expectation Maximize Algorithm}
\index{E-M Algorithm (Expectation Maximize Algorithm)}
    ...
    % Take two-component gaussion mixture model $ k=2 $ as example.
     ...
     Assumption and notation: $ X $ is generated from a mixed distribution of $ k $ normal:
    
    \[
        X\sim \sum_{l=1}^k\pi_lN(\mu_l,\Sigma _l)=\sum_{l=1}^k\pi_lN(\theta _l),\, \sum_{l=1}^k\pi_l=1,\,\pi_l\geq 0.
    \]
    
    Use its likelihood function $ L(\theta;x) $ and maximize posterior probability by $ \dfrac{\partial^{} \ell}{\partial \theta ^{}} $:
    \[
        L(\{\theta_l \};x)=\prod_{i=1}^N \sum_{l=1}^k\pi_l   \dfrac{1}{(2\pi)^{p/2}|\Sigma _l|^{1/2}}\exp\left(   -\dfrac{1}{2}(x-\mu _l)'\Sigma _l(x-\mu _l)\right)
    \]
    
    Extreme value with constraint $ \sum_{l=1}^k \pi_l=1 $ requires 
    \[
         \dfrac{\partial^{} \ln L}{\partial \mu _l^{}}=0\quad \dfrac{\partial^{} \ln L}{\partial \Sigma _l^{}}=0 \quad \dfrac{\partial^{} L+\lambda (\sum\limits_{l=1}^k\pi_l-1)}{\partial \pi_l^{}}=0
    \]
    
    Result:
    \begin{align*}
        % &\sum_{i=1}^N\dfrac{\pi_l\phi _{\theta_l}(x_i)}{\sum\limits_{j=1}^k\pi_j\phi _{\theta_j}(x_i)}(x_i-\mu _i)=0\\
        \text{denote:}& \gamma _{il}:=\dfrac{\pi_i\phi _{\theta_l}(x_i)}{\sum\limits_{j=1}^k\pi_j\phi _{\theta_j}(x_i)}\\
        &\begin{cases}
        \mu _l=&\dfrac{\sum\limits_{i=1}^N\gamma _{il}x_i}{\sum\limits_{i=1}^N\gamma_{il}}\\
        \Sigma _l=&\dfrac{\sum\limits_{i=1}^N\gamma _{il}(x_i-\mu _l)(x_i-\mu _l)'}{\sum\limits_{i=1}^N\gamma _{il}}\\
        \pi_l=&\dfrac{1}{N}\sum_{i=1}^N\gamma_{il}
        \end{cases}
    \end{align*}
...
\begin{enumerate}[topsep=2pt,itemsep=2pt]
    \item Use e.g. $ k $-means method to get an initial estimation to $ \hat{\theta }_l=(\hat{\mu}_l,\hat{\Sigma }_l),\,\hat{\pi}_l=1/k$;
    \item $ \mathrm{E_{xpectation}} $-Step: Compute weight(responsibility) of each point;
    \[
        \hat{\gamma }_i(x_i\in G_l)=\dfrac{\hat{\pi}_l\phi _{\hat{\theta }_l}}{\sum\limits_{j=1}^l\hat{\pi}_j\phi _{\hat{\theta}_j}},\, 1\leq i\leq N
    \]
    \item $ \mathrm{M_{aximize}} $-Step: Re-calculate parameters $ \{\mu_l,\Sigma _l,\pi_l\} $;
    \item Repeat step 2,3 until convergence.
    
\end{enumerate}

    Note: EM method for Gaussion Mixture Model is a greedy algorithm -> local maximum.
    
\subsubsection{DBSCAN \& OPTICS Clustering Algorithm}
...
\end{point}
    ...
    Notation:
    \begin{itemize}[topsep=2pt,itemsep=2pt]
        \item Neighbourhood of point $ x_i $:
        \[
        ...
        \[X_{noi}\equiv\displaystyle{\complement_X^{X_{c}\cup X_{bd}} }\]
        \end{itemize}
...
\end{enumerate}
    ...
    Comment: OPTIC is more stable than DBSCAN, capable of dealing with multi-density clustering.
