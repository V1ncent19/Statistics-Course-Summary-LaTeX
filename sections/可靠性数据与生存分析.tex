\chapter{可靠性数据与生存分析部分}\label{SecReliabilityAndSurvivalAnalysis}
\begin{center}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    ...
    \begin{itemize}[topsep=2pt,itemsep=0pt]
        ...
        \end{equation}
        \item Cumulative Hazard Function\index{Cumulative Hazard Function} $ \Lambda (t) $ (or in some materials denoted $ H(t) $):
        \begin{equation}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    \item Mean Survival Time:\index{Mean Survival Time}
    \begin{equation}
    ...
    \item Mean Residual Life Time ($ \mathrm{mrl}  $)\index{Mean Residual Life Time}:\index{Mean Residual Life Time}
    \begin{equation}
    ...
    \end{equation}
    \item Hazard Rate: for two survival r.v. $ T_1,\,T_2 $, the hazard rate at $ t $\index{Hazard Rate}
    \begin{equation}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    ...
    \end{align}
    \item Weibull\index{Weibull Distribution}\index{Distribution!Weibull Distribution} $T\sim W(p,\lambda )=\left[\varepsilon (\lambda ^p)\right]^{1/p}$, degrade to exponential $ \varepsilon (\lambda ) $ when $ p=1 $\footnote{Weibull distribution could also be parameterized as $ W(p,\gamma ) $, where $ \gamma =1/\lambda  $ is the scale factor.}
    \begin{align}
    ...
    \end{align}
    \item Generalized Gamma $T\sim \mathrm{GG}(\alpha ,p,\lambda )$, degrade to Weibull when $ \alpha =p $, to Gamma when $ p=1 $\index{Generalized Gamma Distribution}\index{Distribution!Generalized Gamma Distribution}
    \begin{align}
...
\subsection{Non-Parametric Estimation to Survival Function}
...
and obtain the MLE estimation of $ \hat{\lambda }_i|r_i,d_i $\index{Greenwood's Formula}\footnote{Here we use the $ \Delta  $ method for estimating the variance of function of r.v.: if $ X\sim f(\mu ,\sigma ^2) $:
\begin{align}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    ...
    \end{equation}
    \item Log-Log pointwise approach\index{Log-Log Pointwise Approach} (\lstinline|R.| default): using $ \hat{L}(t)=\log\left[-\log \hat{S}(t)\right]=\log\left[\hat{\Lambda }(t)\right] $
    \begin{equation}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    \item Recap: \index{Pearson's $ \chi^2 $ Test}Pearson's $ \chi^2 $ test: assign $ n $ sample into $ k $ groups, and conduct test on $ p_i $, $ i=1,2,\ldots,k $, denote that $ v_i $ samples are assigned to the $ i^\mathrm{th}  $ groups, then
    \begin{equation}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    \item Wilcoxon Two-Sample Rank Sum Test: Knowledge of Wilcoxon two-sample rank sum test see \index{Wilcoxon Two-Sample Rank Sum Test}\autoref{SubSectionIntroToNonParametricHypothesisTesting}. Recap: to test the distribution difference of $ \vec{X}=(X_{1},X_{2},\ldots,X_{n})  $ and $ \vec{Y}=(Y_{1},Y_{2},\ldots,Y_{m})  $, we mix them together  and rank as $ \vec{Z}=(Z_{(1)},Z_{(2)},\ldots,Z_{(m+n)})$. Rank of $ X_i $:
    \begin{align}
...
\item Mann-Whitney-Wilcoxon rank sum test for censored data: \index{Mann-Whitney-Wilcoxon Rand Sum Test}
...
Basic assumption on dependence form: $ T $ hazard part and covariants part are Separatable:\index{PH Model (Cox's Proportion Hazard Model)}\index{Cox's Proportion Hazard Model}
\begin{equation}
...
The unknown components are $ \lambda _0(t),\beta  $, where the $ \lambda _0(t) $ lies in the $ dim\to\infty $ space, and causes difficulty in conducting inference. Solution: decompose full likelihood into two parts, in which one of them, \textbf{Partial Likelihood} $ L_\mathrm{ PH }(\beta ;X)  $ is only function of $ \beta  $:\index{Partial Likelihood}
\begin{align}
...
Illustration for $ \blue\displaystyle\sum_{i=1}^n\lambda _i\sum_{j=i}^ne^{\beta 'z_j}=\sum_{i=1}^n\sum_{j=1}^i\lambda _je^{\beta 'z_i} $ (Abel's Lemma for Summation by Parts\index{Abel's Lemma for Summation by Parts})
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    \item Cox's modification:\index{Cox's Modification}
    \begin{equation}
...
\begin{itemize}[topsep=2pt,itemsep=0pt]
    \item $ \log $-$ \log $ plots\index{Log-Log Plot}: for categorical $ z_1 $, $ z_2 $, use relation
    \begin{align}
    ...
    \item Goodness-of-fit using Schoenfeld residuals\index{Goodness-of-fit Test}\index{Schoenfeld Residuals}
    \begin{align}
    ...
    \end{align}
    \item (Generalized) Cox-Snell Residual for overall goodness-of-fit: \index{Cox-Snell Residuals}\index{Generalized Cox-Snell Residuals}
